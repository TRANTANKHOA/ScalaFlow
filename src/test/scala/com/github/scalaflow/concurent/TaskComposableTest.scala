package com.github.scalaflow.concurent

import java.time.LocalDateTime

import org.scalatest.{BeforeAndAfterEach, FunSuite}
import org.spark_project.jetty.util.MultiException
import Composable.Bind
import ResumableTask.KeyManager
import Task.{FutureTried, TaskRepeatable}
import TaskComposableTest.{MockKeys, concurrentNotes, _}
...
object TaskComposableTest {
  // some keys
  val ignoredKeys: Seq[String] = Seq("D", "E")
  val alphabet: Seq[String] = Seq("a", "b", "c", "d", "e")
  ...
  object MockKeys extends KeyManager {
    ...
    def print: String = keys.toSeq.sortBy(_._1).map(each => s"${each._1} - ${each._2}").mkString(" | ")
  ...
  object concurrentNotes {
    val map: SortedMap[String, ArrayBuffer[String]] = SortedMap(
      (integers.map(_.toString) ++ alphabet ++ alphabet.map(_.toUpperCase) :+ rootKey :+ finalKey)
...
class TaskComposableTest extends FunSuite with BeforeAndAfterEach {
  ...
  override def beforeEach(): Unit = {
    ...
    alphabetTasks = alphabet.map(str => {
      ...
      integerTasks.zipWithIndex
        .foreach(task => aTask.dependsOn(task._1)
    ...
    unitTasks = alphabetTasks
      .zip(alphabet.map(_.toUpperCase))
      .map(task => {
        task._2 -> task._1.mapWith(task._2, routine = {
          str => concurrentNotes(task._2).append(str.toUpperCase)
        })
  ...
  def none[A](seq: TraversableOnce[A])(predicate: A => Boolean): Boolean = !seq.exists(predicate)
  ...
  test("testStartedFlag that no task should be executed at this point despite calling `run` when declaring `unitTasks`") {
    ...
    assert(concurrentNotes.isEmpty &&
      none(integerTasks)(_.hasStarted) &&
      none(alphabetTasks)(_.hasStarted) &&
      none(unitTasks.values)(_.hasStarted))
  }
  ...
  test("testDependenciesResolution that no task could register a dependency on the finalTask") {
    ...
    assert(
      none(upstreamTasks)(task => Try(task dependsOn finalTask) isSuccess)
    )
  }
  ...
  test("testAndThen should not repeat any computation") {
    ...
    assert(
      none(concurrentNotes.values.groupBy(identity).values)(_.length != 1)
    )
  }
  ...
  test("testDependedBy that `root` task is executed only once and cannot depend on any integerTasks") {
    ...
    assert(
      none(integerTasks)(task => Try(rootTask dependsOn task) isSuccess)
    )
